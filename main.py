...
def plot_orders_rate_hourly(time_axis: np.ndarray,
    ...
    avg_hourly = (b_hourly + m_hourly) / 2
...
def plot_priority_completion_times(
    ...
    Each priority has 2 bars: Baseline, ML
    """
    priorities = ["P1", "P2", "P3"]
...
def plot_all(time_axis_hours: np.ndarray,
    ...
    plot_metric(time_axis_hours, agg_baseline["total_completed"], agg_ml["total_completed"], f"{scope_title}: Total Completed Orders", "Orders", f"{scope}_total_completed.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["pending_orders"], agg_ml["pending_orders"], f"{scope_title}: Pending Orders", "Orders", f"{scope}_pending_orders.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["orders_received"], agg_ml["orders_received"], f"{scope_title}: Orders Received (per Step)", "Orders", f"{scope}_orders_received.png", scope, detected_peaks)
    
    plot_metric(time_axis_hours, agg_baseline["avg_time_all"], agg_ml["avg_time_all"], f"{scope_title}: Avg Completion Time (All) - Rolling 1h Window", "Steps", f"{scope}_avg_time_all.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["coord_overhead"], agg_ml["coord_overhead"], f"{scope_title}: Coordination Overhead", "Wait / (Wait + Move)", f"{scope}_coord_overhead.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["total_wait_steps"], agg_ml["total_wait_steps"], f"{scope_title}: Total Wait Steps", "Steps", f"{scope}_total_wait_steps.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["total_move_steps"], agg_ml["total_move_steps"], f"{scope_title}: Total Move Steps", "Steps", f"{scope}_total_move_steps.png", scope, detected_peaks)

    # Priority-specific completion times
    plot_metric(time_axis_hours, agg_baseline["avg_time_P1"], agg_ml["avg_time_P1"], f"{scope_title}: Avg Completion Time (Normal Priority) - Rolling 1h Window", "Steps", f"{scope}_avg_time_normal.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["avg_time_P2"], agg_ml["avg_time_P2"], f"{scope_title}: Avg Completion Time (High Priority) - Rolling 1h Window", "Steps", f"{scope}_avg_time_high.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["avg_time_P3"], agg_ml["avg_time_P3"], f"{scope_title}: Avg Completion Time (Urgent Priority) - Rolling 1h Window", "Steps", f"{scope}_avg_time_urgent.png", scope, detected_peaks)

    # Priority-specific counts
    plot_metric(time_axis_hours, agg_baseline["count_P1"], agg_ml["count_P1"], f"{scope_title}: Normal Priority Orders Completed", "Orders", f"{scope}_count_normal.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["count_P2"], agg_ml["count_P2"], f"{scope_title}: High Priority Orders Completed", "Orders", f"{scope}_count_high.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["count_P3"], agg_ml["count_P3"], f"{scope_title}: Urgent Priority Orders Completed", "Orders", f"{scope}_count_urgent.png", scope, detected_peaks)

    # Battery metrics
    plot_metric(time_axis_hours, agg_baseline["avg_battery"], agg_ml["avg_battery"], f"{scope_title}: Average Robot Battery Level", "Battery %", f"{scope}_avg_battery.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["min_battery"], agg_ml["min_battery"], f"{scope_title}: Minimum Robot Battery Level", "Battery %", f"{scope}_min_battery.png", scope, detected_peaks)
    plot_metric(time_axis_hours, agg_baseline["charging_robots"], agg_ml["charging_robots"], f"{scope_title}: Number of Robots Charging", "Robots", f"{scope}_charging_robots.png", scope, detected_peaks)
...
def run_benchmark(seeds: List[int], steps: int, scope_title: str, scope: str):
    ...
    # Plot
    plot_all(time_axis_hours, agg_b, agg_m, scope_title, scope)
